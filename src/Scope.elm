...
importVisitor { setter, getter } maybeVisitor ((Node range import_) as node) outerContext =
    case import_.moduleAlias of
        Nothing ->
            case maybeVisitor of
                ...
                Just fn ->
                    fn node outerContext

        Just alias_ ->
            let
                innerContext : { scopes : Nonempty Scope, importAliases : Dict String (List String) }
                innerContext =
                    getter outerContext
                        |> unbox
            ...
            case maybeVisitor of
                ...
                Just fn ->
                    fn node newOuterContext


unbox : Context -> { scopes : Nonempty Scope, importAliases : Dict String (List String) }
...
moduleDefinitionVisitor : Node Module -> Context2 -> ( List Error, Context2 )
...
expressionVisitor : Node Expression -> Direction -> Context2 -> ( List Error, Context2 )
...
declarationVisitor : Node Declaration -> Direction -> Context2 -> ( List Error, Context2 )
...
markUsedTypesAndModules : { types : List String, modules : List String } -> Context2 -> Context2
...
finalEvaluation : Context2 -> List Error
...
registerFunction : LetBlockContext -> Function -> Context2 -> Context2
...
register : VariableInfo -> String -> Context2 -> Context2
...
registerModule : VariableInfo -> String -> Context2 -> Context2
registerModule variableInfo name context =
    { context | declaredModules = Dict.insert name variableInfo context.declaredModules }


registerVariable : VariableInfo -> String -> Context2 -> Context2
registerVariable variableInfo name context =
    let
        scopes : Nonempty Scope
        scopes =
...
markAllAsUsed : List String -> Context2 -> Context2
markAllAsUsed names context =
    List.foldl markAsUsed context names


markAsUsed : String -> Context2 -> Context2
markAsUsed name context =
    let
        scopes : Nonempty Scope
        scopes =
