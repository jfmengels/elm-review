...
b = a 1"""
    ...
    , test "should report unused top-level variables" <|
        \() ->
            testRule """module A exposing (b)
a = 1"""
                |> TestUtil.expectErrors [ Error.create "Variable `a` is not used" (TestUtil.location ( 2, 1 ) ( 2, 2 )) ]
    , test "should report unused top-level variables even if they are annotated" <|
        \() ->
            testRule """module A exposing (b)
a: Int
a = 1"""
                |> TestUtil.expectErrors [ Error.create "Variable `a` is not used" (TestUtil.location ( 3, 1 ) ( 3, 2 )) ]
...
b = 2
c = 3"""
                |> TestUtil.expectErrors [ Error.create "Variable `c` is not used" (TestUtil.location ( 4, 1 ) ( 4, 2 )) ]
...
a = let b = 1
    in 2"""
                |> TestUtil.expectErrors [ Error.create "Variable `b` is not used" (TestUtil.location ( 2, 9 ) ( 2, 10 )) ]
...
import Foo exposing (a)"""
                |> TestUtil.expectErrors [ Error.create "Imported variable `a` is not used" (TestUtil.location ( 2, 22 ) ( 2, 23 )) ]
...
a = case thing of
    ...
    , test "should report unused custom type declarations" <|
        \() ->
            testRule """module A exposing (a)
type A = B | C"""
                |> TestUtil.expectErrors [ Error.create "Type `A` is not used" (TestUtil.location ( 2, 6 ) ( 2, 7 )) ]
    , test "should report unused type aliases declarations" <|
        \() ->
            testRule """module A exposing (a)
type alias A = { a : B }"""
                |> TestUtil.expectErrors [ Error.create "Type `A` is not used" (TestUtil.location ( 2, 12 ) ( 2, 13 )) ]
...
a : { r | c: A }
a str = {c = str}"""
                |> TestUtil.expectErrors [ Error.create "Variable `r` is not used" (TestUtil.location ( 2, 1 ) ( 2, 2 )) ]
    , test "should report unused operator import" <|
        \() ->
            testRule """module A exposing (a)
import Parser exposing ((</>))"""
                |> TestUtil.expectErrors [ Error.create "Imported operator `</>` is not used" (TestUtil.location ( 2, 25 ) ( 2, 30 )) ]
...
a = 1"""
    ...
    , test "should report unused import" <|
        \() ->
            testRule """module A exposing (a)
import Html"""
                |> TestUtil.expectErrors [ Error.create "Imported module `Html` is not used" (TestUtil.location ( 2, 8 ) ( 2, 12 )) ]
    , test "should report unused import (multiples segments)" <|
        \() ->
            testRule """module A exposing (a)
import Html.Styled.Attributes"""
                |> TestUtil.expectErrors [ Error.create "Imported module `Html.Styled.Attributes` is not used" (TestUtil.location ( 2, 8 ) ( 2, 30 )) ]
...
a = Html.Styled.Attributes.href"""
                |> TestUtil.expectErrors [ Error.create "Variable `href` is not used" (TestUtil.location ( 2, 1 ) ( 2, 5 )) ]
...
a = Html.href"""
    ...
    , test "should report unused import alias" <|
        \() ->
            testRule """module A exposing (a)
import Html.Styled.Attributes as Html"""
                |> TestUtil.expectErrors [ Error.create "Module alias `Html` is not used" (TestUtil.location ( 2, 34 ) ( 2, 38 )) ]
...
port input : (Json.Decode.Value -> msg) -> Sub msg"""
                |> TestUtil.expectErrors
                    [ Error.create "Port `input` is not used (Warning: Removing this port may break your application if it is used in the JS code)" (TestUtil.location ( 3, 6 ) ( 3, 11 ))
                    ]
...
port output : Json.Encode.Value -> Cmd msg"""
                |> TestUtil.expectErrors
                    [ Error.create "Port `output` is not used (Warning: Removing this port may break your application if it is used in the JS code)" (TestUtil.location ( 3, 6 ) ( 3, 12 ))
                    ]
    ]
